...
function plot_perc_change_by_cell_name(dataTable, analysisFolder, targetVgsStr)
    ...
    for c = 1:length(cellNames)
        ...
        if length(parts) > 1
            % Handle names with underscores (e.g., 'Pizza_A_Slice_1')
            letterGroup = parts{2};
        else
            % Handle names without underscores (e.g., 'A1', 'B2')
            if ischar(cellName) && ~isempty(cellName)
                letterGroup = cellName(1);
            else
                warning('Could not parse group from cell name: %s. Skipping.', cellName);
                continue;
            end
        end
        
        % Initialize the group entry if it doesn't exist
        if ~isKey(groupData, letterGroup)
            % Corrected: Initialize the struct and then assign to the map
            newGroup = struct('Concentrations', [], 'PercChanges', []);
            newGroup.Concentrations = currentTable.Concentration;
            groupData(letterGroup) = newGroup;
        end
        
        % Get the current data for the group
        currentGroup = groupData(letterGroup);
        
        % Append the percentage change data from the current cell to the group's data
        currentGroup.PercChanges = [currentGroup.PercChanges, currentTable.AvgPercentageChange];
        
        % Update the map
        groupData(letterGroup) = currentGroup;
    end
    
    % Now, iterate through the aggregated data to create plots and tables
