...
function plot_avg_sweeps_by_run(dataTable, masterFolder)
    ...
    % Description: This script creates a single plot for each measurement run,
    % showing the average current vs. voltage for all concentrations within that run.
    % It also saves the summary data to an Excel file.
    %
    % This is a converted version of the original function provided, made to be a standalone script.
    % It includes placeholder data and a dummy folder structure for demonstration purposes.
    
    % --- SCRIPT SETUP: DEFINE PLACEHOLDER DATA AND FOLDERS ---
    % A real-world application would get these from a data loading function.
